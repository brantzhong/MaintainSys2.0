...
//        : _typeof(exports)) === 'object' && typeof module !== 'undefined' ? module.exports = factory()
 //       : typeof define === 'function' && define.amd ? define(factory)
 //       : window.imageConversion = factory();

var imageConversion = (function () {
    ...
    var imagetoCanvas = async function (image) {
    ...
    var canvastoFile = function (canvas, quality) {
    ...
    var canvastoDataURL = async function (canvas, quality, type) {
    ...
    var dataURLtoFile = async function (dataURL, type) {
    ...
    var downloadFile = function (file, fileName) {
    ...
    var compress = async function (file) {
        var config = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
        ...
        var dataURL = filetoDataURL(file);
        dataURL.then(function(dataURL_res){
            var originalMime = dataURL_res.split(',')[0].match(/:(.*?);/)[1]; // 原始图像图片类型
            ...
            image.then(function(image_res){
                var canvas = imagetoCanvas(image_res, Object.assign({}, config));
                canvas.then(function(canvas_res){

                    var compressDataURL = canvastoDataURL(canvas_res, config.quality, mime);

                    compressDataURL.then(function (compressDataURL_res) {
                        var compressFile = dataURLtoFile(compressDataURL_res, originalMime);
                        return compressFile;
                    });
                });
    ...
    var compressAccurately = async function (file) {
        var config = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};
        ...
        var image = await dataURLtoImage(dataURL);
        var canvas = await imagetoCanvas(image, Object.assign({}, config));
        /**
        ...
        var compressFile = await dataURLtoFile(compressDataURL, originalMime); // console.log("最终图片大小：", compressFile.size);
    ...
    return {
        ...
        compress             :compress          ,
        compressAccurately   :compressAccurately,
